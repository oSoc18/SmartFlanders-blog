...
- The data is not readable by machines. They have a Text-to-Speech engine on their website for deaf people, it won't see the difference.
...
They will define their own vocabulary to do this, the data is better published but not machine readable and their reports are very long to see if the toilet is accessible or not.

We handle this problem differently by using Linked Open Data, we link several datasets together to provide the user an unifying way to access all the data he or she needs.
...
The accessiblity data can be published for example by city, Toegankelijkheid Vlaanderen, ... while it's still connected to the opening hours.
The data is now also machine readable by using RDF ([Resource Description Framework](https://en.wikipedia.org/wiki/Resource_Description_Framework)) to describe what the data means, it provides context.
Using RDF we can define exactly what '*accessibility*' is and let other parties reuse the vocabulary. If a machine reads the vocabulary it can understand what the data means. This wasn't the case with the bolded public toilets in our example above.
...
1. You need to lookup the location of the public toilets
...
3. The openingshours are mentioned on some other pages as well.
